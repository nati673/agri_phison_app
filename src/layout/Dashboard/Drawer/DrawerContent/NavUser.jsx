  ...
  return (
    <Box sx={{ p: 1.25, px: !drawerOpen ? 1.25 : 3, borderTop: '2px solid ', borderTopColor: 'divider' }}>
      <List disablePadding>
        ...
        >
          <ListItemAvatar>
            <Avatar
              alt="Avatar"
              src={user?.profile_image ? `${import.meta.env.VITE_APP_API_URL}/user/profile/${user?.profile_image}` : avatar1}
              // src={avatar1}
              sx={{ ...(drawerOpen && { width: 46, height: 46 }) }}
            />
          </ListItemAvatar>
          <ListItemText primary={user?.name} sx={{ ...(!drawerOpen && { display: 'none' }) }} secondary={user?.email} />
        </ListItem>
      </List>
      <Menu
        id="basic-menu"
      ...
      >
        ...
        <MenuItem component={Link} to="/work/profiles/user/personal" onClick={handleClose}>
          Profile
        </MenuItem>
        <MenuItem component={Link} to="/work/profiles/account/basic" onClick={handleClose}>
          My account
        </MenuItem>
      </Menu>
    </Box>

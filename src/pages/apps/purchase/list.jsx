...
import { filterPurchases } from 'api/purchase';
...
import { Add, ArrowDown2, ArrowUp2, Edit, ShoppingBag, Trash } from 'iconsax-react';
import AlertProductDelete from 'sections/apps/purchase/AlertProductDelete';
import PurchaseView from 'sections/apps/purchase/PurchaseView';
                    ...
                    {row.getIsExpanded() && (
                      <TableRow sx={{ '&:hover': { bgcolor: `${backColor} !important` } }}>
                        <TableCell colSpan={row.getVisibleCells().length}>
                          <PurchaseView data={row.original} />
                        </TableCell>
                      </TableRow>
                    )}
                  </Fragment>
...
export default function ProductsPage() {
  ...
  const filterData = async () => {
    const response = await filterPurchases(user?.company_id, filter);
  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row, getValue }) => {
          ...
          return (
            <Stack direction="row" spacing={1.5} alignItems="center">
              <ShoppingBag size="32" color={theme.palette.primary.main} variant="Bulk" />
              <Stack spacing={0}>
                <Typography variant="body1" fontWeight={600}>
                  {getValue()}
                </Typography>
                <Typography>
                  {row.original.supplier} • {date}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {row.original.business_unit} | {row.original.location_name}
                </Typography>
              </Stack>
            </Stack>
          );
      ...
      {
        header: 'Item',
        cell: ({ row }) => <Typography variant="body2">Items {row.original.items.length || 0}</Typography>
      },
      {
        header: 'Total Cost',
      ...
      },
      {
        ...
        cell: ({ getValue }) => <Typography color="text.secondary">{new Date(getValue()).toLocaleDateString()}</Typography>
      },
      {
        header: 'Actions',
        meta: { className: 'cell-center' },
              ...
              <Tooltip title="Delete">
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleCloseDelete();
                    setProductDeleteId(Number(row.original.purchase_id));
                  }}
                >
                  <Trash />
                </IconButton>
